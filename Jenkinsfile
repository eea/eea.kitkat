pipeline {
  ...
  environment {
        GIT_NAME = "eea.kitkat"
        SONARQUBE_TAGS = "demo-www.eea.europa.eu,www.eea.europa.eu-en,www.eea.europa.eu,climate-advisory-board.europa.eu,forest.eea.europa.eu,biodiversity.europa.eu,insitu.copernicus.eu,clmsdemo.devel6cph.eea.europa.eu,land.copernicus.eu,demo-ied.eea.europa.eu,www.ied.eea.europa.eu-en,demo-water.devel5cph.eea.europa.eu-freshwater,water.europa.eu-freshwater,demo-wise.02pre.eea.europa.eu-marine,water.europa.eu-marine,water.europa.eu-msfd"
    }
    ...
    stage('Code') {
      steps {
        parallel(
          ...
          "ZPT Lint": {
            node(label: 'docker') {
              sh '''docker pull eeacms/plone-test:4; docker run -i --rm --name="$BUILD_TAG-zptlint" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone-test:4 zptlint'''
            }
          },

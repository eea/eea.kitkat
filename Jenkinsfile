        ...
        parallel(
          ...
          "Plone6 & Python3": {
             node(label: 'docker') {
              ...
              sh '''docker run -i --rm --name="$BUILD_TAG" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="zope-testrunner $GIT_NAME[test]" -e GIT_USER="eea" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone-test:6'''
            }
           }
        )
      }

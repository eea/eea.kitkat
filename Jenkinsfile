        ...
        node(label: 'swarm') {
          script {
            ...
            withSonarQubeEnv('Sonarqube') {
              sh '''sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; \
                sonar-scanner \
                -Dsonar.python.xunit.skipDetails=true \
                -Dsonar.python.xunit.reportPath=coverage/junit-results/testreports/*.xml \
                -Dsonar.python.coverage.reportPaths=coverage/coverage.xml \
                -Dsonar.sources=. \
                -Dsonar.exclusions=**/tests/**,**/setup.py \
                -Dsonar.projectKey=$GIT_NAME-$BRANCH_NAME \
